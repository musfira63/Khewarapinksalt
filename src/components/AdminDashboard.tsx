...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (resOrders.ok && dataOrders.success) {
        ...
        setError(dataOrders.error || "Session expired. Please log in again.");
        setIsAuthenticated(false);
      }
    } catch (err) {
      setError("Connection error. Refresh failed.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteOrder = async (orderNumber: string) => {
    if (!confirm(`Are you sure you want to delete order #${orderNumber} from the live logs database? This is irreversible.`)) {
      return;
    }
    try {
      ...
      alert("Connection error. Delete failed.");
    }
  };

  const handleUpdateInquiryStatus = async (inquiryId: string, newStatus: string) => {
    try {
      const response = await fetch("/api/admin/update-inquiry-status", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      if (response.ok && data.success) {
        setInquiries(prev => prev.map(iq => iq.inquiryId === inquiryId ? { ...iq, status: newStatus } : iq));
      } else {
  ...
  const handleDeleteInquiry = async (inquiryId: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok && data.success) {
        setInquiries(prev => prev.filter(iq => iq.inquiryId !== inquiryId));
      } else {
                              ...
                              {order.items.map((item: any, idx: number) => (
                                <div key={idx} className="flex justify-between text-[11px]">
                                  ...
                                  <span className="font-mono text-stone">
                                    {order.currency === "PKR" ? "Rs" : "$"} {((item.unitPrice || 0) * item.quantity).toLocaleString()}
                                  </span>
                                </div>
                              ))}
                            </div>
