...
export const OrderForm: React.FC = () => {
  ...
  const handleCheckoutSubmit = (e: React.FormEvent) => {
    ...
    .then(res => {
      if (!res.ok) {
        throw new Error("Failed to place order. Server returned status: " + res.status);
      }
      return res.json();
    })
    .then(data => {
      ...
      if (data.success && data.order) {
        setOrderReceipt({
          ...
          screenshotName: data.order.screenshotName
        });
    ...
    .catch(err => {
      setIsProcessing(false);
      console.error(err);
      setValidationError("Failed to connect to the order server. Please try again.");
    });
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        type="text"
                        required
                        placeholder="e.g. Karachi or New York"
                        className="bg-ink border border-cream/10 text-cream text-xs rounded p-2.5 focus:outline-none focus:border-salt-pink"
                        value={city}
