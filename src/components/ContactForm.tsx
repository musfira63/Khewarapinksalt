...
export const ContactForm: React.FC<ContactFormProps> = ({ activeSegment }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Store in localStorage & notify Admin Dashboard live
    try {
      const existingLocal = JSON.parse(localStorage.getItem("local_inquiries") || "[]");
      const filtered = existingLocal.filter((iq: any) => iq.inquiryId !== refId);
      filtered.unshift(newInquiryObj);
      localStorage.setItem("local_inquiries", JSON.stringify(filtered));
      window.dispatchEvent(new Event("khewara_new_data"));
    } catch (e) {
      console.warn("Failed to write inquiry to localStorage:", e);
    }
    ...
    try {
      ...
      const finalRefId = data.inquiry?.inquiryId || refId;

      // 2. Prepare Instant Email Dispatch Link
      const subject = encodeURIComponent(`Khewara Pink Salt Inquiry [Ref: ${finalRefId}]: ${reason}`);
      const body = encodeURIComponent(
        `Khewara Pink Salt Customer Inquiry Submission:\n\n` +
        `Inquiry Ref ID: ${finalRefId}\n` +
        `Customer Name: ${name}\n` +
        `Email: ${email}\n` +
      ...
      setSubmittedInquiry({ inquiryId: finalRefId, targetEmail });
      setIsSuccess(true);
      setStatusMessage(`Inquiry recorded in Admin Portal (#${finalRefId})!`);
      ...
      setIsSuccess(true);
      setStatusMessage(`Inquiry recorded (#${refId})!`);
      setTimeout(() => {
        try {
          window.open(`mailto:${targetEmail}?subject=${subject}&body=${body}`, "_blank");
        } catch (e) {}
        ...
        <div className="flex flex-col gap-5 mt-2">
          ...
          <div className="flex items-start gap-4 p-4 rounded bg-ink-3/30 border border-salt-pink/20 hover:border-salt-pink/50 transition-all duration-300 pink-shiny-border">
            ...
            <div className="flex-1">
              <span className="font-mono text-[10px] text-stone uppercase tracking-wider block">Phone &amp; WhatsApp Hotlines</span>
              <div className="flex flex-col gap-2 mt-1">
                <div className="flex flex-wrap items-center gap-3">
                  <a href="tel:+923132213228" className="text-sm text-cream hover:text-salt-pink transition-colors font-mono font-bold">
                    0313 2213228
                  </a>
                  <a
                    href="https://wa.me/923132213228?text=Hello%20Khewara%20Pink%20Salt%2C%20I%20have%20an%20inquiry"
                    target="_blank"
                ...
                <div className="flex flex-wrap items-center gap-3 border-t border-cream/5 pt-1.5">
                  <a href="tel:+923343711613" className="text-sm text-stone hover:text-salt-pink transition-colors font-mono font-bold">
                    0334 3711613
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded bg-cream/10 text-cream font-mono text-[10px] hover:bg-cream hover:text-ink transition-all"
                  >
                    <WhatsAppIcon size={12} />
                    <span>WhatsApp</span>
                  </a>
